...
{A rotina "record"
basicamente é um comando que registra um tipo de variavel
que tem mais de uma variavel e/ou mais de um tipo,
representa um conjunto de variaveis}

{O comando Type serve para criar um tipo de variavel, que
vai ser usada mais de uma vez (apesar de ser possivel 
usar uma vez)}
...
var
ultpont:integer;//variavel que guarda a ultima pontuação
conf:confirma;//variavel que compara o registro do login com o digitado
login:registro;//guarda o valor dos dados do usuario na pagina registro
senha:string[16];// variavel global para guardar a senha e transferir para outros procedimentos
HsAtualofN:string;//guarda o nome referente a pessoa que fez uma pontuação (o mesmo valor do "login.user") 
menu,erro,//variaveis usadas no menu para controlar a validade da entrada feita nos menus
goX,goY,goXJ,goYJ,//usadas para localizar e ir a um detrerminado ponto da tela 
HsAtual:integer; //guarda a pontuação atual
confLogBool:boolean;//usada para verificar se o login foi bem sucedido
menui,confirmar:char;// controlar menus, armazenar valores das escolhas do usuario
cadfile:file of registro;//armazena os valores do registro
highscoreA:array [1..5] of integer;//guarda os valores das maiores pontuações 
highscoref:file of array [1..5] of integer;//armazena os valores das maiores pontuações
highscoreAofN:array [1..5] of string;//guarda os nomes (referente ao "login.user") das maiores pontuações 
highscorefofN:text;//armazena os nomes (referente ao "login.user") das maiores pontuações
...
var n:integer;
begin
  textcolor(yellow);//diz a cor do frame
  gotoxy(1,1); write(#201); //coloca o cato
	for n:= 1 to 78 do write(#205); //repete 78 vezes a introdução do caractere que forma a linha superior (discontando os cantos) 
	write(#187); //coloca o outro canto //top do frame
	
	for n:=2 to 24 do begin //borda direita e esquerda (repete 24 vezes e cada vez que execulta, pula uma linha referente a n)
		gotoxy(1,n);
		write(#186);
		gotoxy(80,n);
		write(#186);  
  ...
  gotoxy(1,25); write(#200);//coloca o cato
	for n:= 1 to 78 do write(#205);//repete 78 vezes a introdução do caractere que forma a linha inferior (discontando os cantos) 
  write(#188);//coloca o outro canto //parte inferior do frame
  gotoxy(1,1);textcolor(white);//volta para a primeira linha do prompt e volta a configuração normal do programa;
end;

//Material de estudo
procedure matEstudo ;
var goxt,goyt,nl{Numero da Linha}:integer;//variavel responsavel exclusivamente para controlar o texto
Begin
  goxt:=3;//variavel responsavel exclusivamente para controlar o texto
  goyt:=2;
  frame;
  gotoxy(3,24);write(' ',#30,' ',#31,' controlam a rolagem do texto                   <ESC> volta para o menu');
  repeat
    nl:=0;
    { //exemplo do codigo de exibição de linha "aberto" e identado.
      //o valor onde o texto começa ("goxt" e "goyt") mais o numero
      //da linha (nl) não pode ser menor ou igual a dois e nem maior ou igual a 22
    if (goyt+nl>=2) and (goyt+nl<=22) then begin //essa linha checa se o string pode entrar na tela ou não baseada na possição em Y
      gotoxy(goxt,goyt+nl);//direciona o dursor
      write(' Exemplo de string que iria aqui ');//escreve a linha
    end;
    nl:=nl+1; //adiciona mais um reverente ao numero da linha, sendo
    assim, na proxima linha, invés de goyt+nl ser igual a 2, será igual a 3
    }
  ...
  if (goyt+nl>=2) and (goyt+nl<=22) then begin gotoxy(goxt,goyt+nl);write('                                                                            ');end;nl:=nl+1;
    ...
    gotoxy(1,1);//ao final de cada execução, volta o cursor para a primeira linha
    case (readkey) of //analisa qual tecla foi precionada
      #80{seta para baixo}:if (goyt>=(nl*-1)+16) then goyt:=goyt-1; //texto sobe
      #72{seta para cima}:if (goyt<=1) then goyt:=goyt+1; //texto desce
      #27:break;//sai do looping, texto
    end;
  until 2<>2;
end;

//regras/how_to do jogo //Mesmo princípio do "procedure matEstudo;"
procedure Srule ;
...
begin
  ...
  close(HighscoreF);
  if (ultpont>0) then begin
	gotoxy(35,18);write('Ultima pontuação: ',ultpont,' ',conf.user);
  end;
	gotoxy(35,20);write('Pressione <ESC> para voltar');
  if readkey=#27 then else highscore;
end;

//mostra a imagem do mundo
...
begin
  ...
  repeat
    ...
    goX:=42;goY:=14;hiddensenha;conf.senha:=senha;
    clrscr;
